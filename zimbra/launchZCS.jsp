...
<script>
	...
	var eventdbl = new MouseEvent('dblclick', {
	'view': window,
	'bubbles': true,
	'cancelable': true
	});
	function loadOneClick(){
		if(document.querySelectorAll(".DwtListView table .DwtListView-Rows li").length){
			document.querySelectorAll(".DwtListView table .DwtListView-Rows li").forEach((item)=>{
				if(!item.classList.contains('oneclick')){
					item.classList.add('oneclick');
					item.addEventListener("click", (e)=>{
						e.preventDefault();
						item.dispatchEvent(eventdbl);
					});
				}
			});
		}
